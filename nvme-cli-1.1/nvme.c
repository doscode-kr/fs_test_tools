...
// FOR VU command(WAI) of SK hinyx SSD
// get_wai_information
static int get_wai_information(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	const char *desc = "Retrieve WAI information for the given device "\
	...
	// getpagesize() = 4096
	if (cfg.vu_desc_len == 0){
	    cfg.vu_desc_len = 512;
	    if (posix_memalign(&buf, getpagesize(), cfg.vu_desc_len))
		exit(ENOMEM);
	    memset(buf, 0, cfg.vu_desc_len);
	}

	vu_desc = (__u32 *)buf;
	vu_desc[0] = 0x1B003; //VU_OPCODE_SMART_GET_WAI_INFO;


	err = nvme_wai_information(fd, cfg.namespace_id, cfg.vu_desc_len, buf);
	if (!err) {
		if (fmt == BINARY)
			fprintf(stderr, "this does not support Binary mode\n");//d_raw((unsigned char *)&smart_log, sizeof(smart_log));
		else if (fmt == JSON) 
			fprintf(stderr, "this does not support json mode\n");//json_smart_log(&smart_log, cfg.namespace_id, devicename);
		else 
			show_get_wai_information(buf, cfg.namespace_id, devicename);
	}
	else if (err > 0)
		fprintf(stderr, "NVMe Status:%s(%x)\n",
					nvme_status_to_string(err), err);
	else
		perror("get wai information");
	return err;
}
